...
export const Resume: React.FC<{ className?: string }> = ({
  ...
  return (
    <article
      className={`prose prose-sm lg:prose-base max-w-none print:prose-lg print:mx-6 ${className}`}
    ...
    >
      ...
      <header className="flex items-start justify-between gap-4">
        <div>
          <h1 className="text-2xl text-[var(--text)] font-extrabold tracking-tight">{personal.name}</h1>
          {personal.title && (
            <p className="text-sm text-[var(--muted)] mt-1">{personal.title}</p>
          )}
          {personal.headline && (
            <div className="text-sm text-[var(--muted)] mt-1">{personal.headline}</div>
          )}
        </div>

        <div className="text-sm text-right space-y-1 print:text-black">
          {contact.email && (
          ...
          {contact.location && (
            <div className="flex items-center justify-end gap-2">
              <LuMapPinHouse className="w-4 h-4" />
        ...
        <section>
          ...
          <div className="space-y-3">
            {resumeInfo.skills.map((group, gi) => (
              <div key={gi}>
                {group.title && <div className="text-sm font-medium mb-1">{group.title}</div>}
                <ul className="flex flex-wrap gap-2 text-sm">
                  {group.skills.map((s) => (
                    ...
                    >
                      {s.name}
                      {s.years ? <span className="ml-2 text-xs text-[var(--muted)]">· {s.years}y</span> : null}
                      {s.level ? <span className="ml-1 text-xs text-[var(--muted)]">· {s.level}</span> : null}
                    </li>
                  ))}
                </ul>
              </div>
                ...
                <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start">
                  <div>
                    <div className="font-medium">
                      {exp.title}
                      {exp.company ? <span className="text-[var(--muted)]"> — {exp.company}</span> : null}
                    </div>
                    {exp.location && <div className="text-xs text-[var(--muted)]">{exp.location}</div>}
                  </div>
                  <div className="text-[var(--muted)] mt-2 sm:mt-0">
                    {typeof exp.date === "string" ? exp.date : `${exp.date?.start ?? ""}${exp.date?.present ? " — Present" : exp.date?.end ? ` — ${exp.date.end}` : ""}`}
                  </div>
                </div>

                {exp.summary && <p className="mt-2 text-[var(--text)]">{exp.summary}</p>}
            ...
            {resumeInfo.projects.map((p) => (
              <div key={p.id ?? p.title} className="text-sm">
                <div className="flex justify-between">
                  <div className="font-medium">{p.title}{p.short ? <span className="text-[var(--muted)]"> — {p.short}</span> : null}</div>
                  <div className="text-[var(--muted)]">
                    {typeof p.date === "string" ? p.date : p.date?.start ?? ""}
                  </div>
                </div>
                {p.description && <div className="mt-1 text-[var(--text)]">{p.description}</div>}
                {p.tags && p.tags.length > 0 && (
                  <div className="mt-2 flex gap-2 flex-wrap">
                    {p.tags.map((t) => (
                      <span key={t} className="text-xs font-semibold px-2 py-1 rounded-full bg-[var(--surface)] border border-[var(--border)]">
                        {t}
                      </span>
                    ))}
                  </div>
        ...
        <section>
          ...
          <div className="space-y-2 text-sm">
            {resumeInfo.education.map((ed) => (
              <div key={ed.school ?? ed.degree} className="flex justify-between">
                <div>
                  {ed.degree ? <strong>{ed.degree}</strong> : null}
                  {ed.school ? <span className="ml-2">{ed.school}</span> : null}
                </div>
                <div className="text-[var(--muted)]">{typeof ed.date === "string" ? ed.date : ed.date?.start ?? ed.date?.end ?? ""}</div>
              </div>
            ))}
          </div>
        </section>
        ...
        <section>
          ...
          <ul className="text-sm list-disc list-inside">
            {resumeInfo.certifications.map((c) => (
              <li key={c.name}>
                <div className="font-medium">{c.name}{c.issuer ? <span className="text-[var(--muted)]"> — {c.issuer}</span> : null}</div>
                {c.date && <div className="text-xs text-[var(--muted)]">{typeof c.date === "string" ? c.date : c.date.start ?? c.date.end}</div>}
                {c.url && (
                  <div>
                    <a href={c.url} target="_blank" rel="noopener noreferrer" className="underline text-sm">
                      Link
                    </a>
                  </div>
                )}
